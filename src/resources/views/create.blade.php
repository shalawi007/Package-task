...
<body>
    <div class="mt-6">
        <div class="grid sm:grid-cols-2 items-start gap-14 p-8 mx-auto max-w-4xl bg-white shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-md font-[sans-serif]">
            ...
            <form class="ml-auo space-y-4" action="/submit/message" method="POST">
                @csrf
                ...
                @error('subject')
                    <div class="text-red-800">{{$message}}</div>
                @enderror
                <textarea placeholder='Type Your Message' rows="6" id="message" name="message"
                    class="w-full text-gray-800 rounded-md px-4 border text-sm pt-2.5 outline-blue-500"></textarea>
                @error('message')
                    <div class="text-red-800">{{$message}}</div>
                @enderror
                <input type='submit' value="Submit"
                    class="text-white bg-blue-500 hover:bg-blue-600 rounded-md text-sm px-4 py-3 w-full !mt-6"></input>
            </form>
            @session('success')
